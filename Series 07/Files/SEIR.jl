...
# ╔═╡ 58c0645d-6287-4fce-9115-ff637260a96f
using DifferentialEquations, Plots, PlutoUI; plotly()

# ╔═╡ 77151be0-5f83-11ed-3c62-4703a2efc1f6
md"# SEIR Model"

# ╔═╡ 0920c68c-650d-4547-b076-03bd4c81050f
md"Disclaimer: This notebook is for coding demonstration purposes only and is not to be used for public policy discussions. Use this notebook at your own risk."

# ╔═╡ 579c0cce-0203-4131-9125-c8e84cc41dea
md"Source: [A Simulation of a COVID-19 Epidemic Based on a Deterministic SEIR Model](https://www.frontiersin.org/articles/10.3389/fpubh.2020.00230/full)"

# ╔═╡ 0dd264ee-7571-4736-ae46-67031ebb9c80
md"## Add Packages"

# ╔═╡ 7264fb5a-35a8-4867-bad1-0852f2f6a7a1
md"## Describe Problem"

# ╔═╡ a63d1c02-8733-44dc-b59a-b8753d31803d
md"### System of First Order Ordinary Differential Equations"

# ╔═╡ 4efb8b7e-9a88-4a63-bdfc-abf618c08e74
md"The total (initial) population, $N_0$, is categorized into four classes, namely, susceptible, $S(t)$, exposed, $E(t)$, infected-infectious, $I(t)$ and recovered, $R(t)$, where $t$ is the time variable. The governing differential equations are"

# ╔═╡ fdadb2f2-98b6-48bb-8a14-729b38544b8c
md"$\begin{align}
\dot{S} &= \Lambda - \mu S - \beta S \frac{I}{N} \\
\dot{E} &= \beta S \frac{I}{N} - (\mu + \epsilon) E \\
\dot{I} &= \epsilon E - (\gamma + \mu + \alpha) I \\
\dot{R} &= \gamma I - \mu R
\end{align}$"

# ╔═╡ b25de58a-0511-48f2-a5b6-eb28681ad363
md"where $N = S + E + I + R \leq N_0$ in this case, and a dot above a variable denotes time differentiation. Equations (1) are subject to the initial conditions $S(0)$, $E(0)$, $I(0)$, and $R(0)$. The parameters are defined as:"

# ╔═╡ 21ea4cd8-c2c5-4dd4-a377-2b554307a6b4
md"""
...
# ╔═╡ 0278dcd5-4f3f-47cf-874b-7f6e09d01d56
md"having units of $(1/T)$, with $T$: time."

# ╔═╡ 07c1d57d-394c-4447-82dc-d56375f09d64
md"#### DifferentialEquations.jl"

# ╔═╡ da0965cb-9d65-4bdd-9a7c-9155386eadbc
md"##### SEIR Model"

# ╔═╡ 5342b159-859a-451d-bab5-4c701ad13b0e
md"$f(du, u, p, t) =
\begin{cases}
du[1] = p[1] - p[2]u[1] - p[3] u[1] \frac{u[3]}{p[4]} \\
du[2] = p[3] u[1] \frac{u[3]}{p[4]} - (p[2] + p[5]) u[2] \\
du[3] = p[5] u[2] - (p[6] + p[2] + p[7]) u[3] \\
du[4] = p[6] u[3] - p[2] u[4]
\end{cases}$"

# ╔═╡ 9b1efb69-59bb-490b-8108-c3e73d9cd80e
md"## Define Julia Function"

# ╔═╡ b99d2432-b690-49f0-9cfb-17e088272735
function SEIR(du, u, p, t)
	
	S, E, I, R = u
	
	Lambda, mu, beta, epsilon, gamma, alpha = p

	N = S + E + I + R
	
	du[1] = Lambda - mu * S - beta * S * I / N
	du[2] = beta * S * I / N - (mu + epsilon) * E
	du[3] = epsilon * E - (gamma + mu + alpha) * I
	du[4] = gamma * I - mu * R

end

# ╔═╡ fcadd52e-5e41-4b7c-8d43-967b7ea21a74
md"## Assign Variables"

# ╔═╡ 753eb00a-63b9-43d4-87d1-59505f6a95a8
md"### u: Unknown Functions S, E, I and R"

# ╔═╡ c78e563d-4fdf-4f36-8a7d-2331a3248f28
md"Note: these initial values are abstract placeholders to be used for model-building purposes only."

# ╔═╡ e6909ece-5ab7-4327-ba90-6cc7c7c9f3eb
S_begin = 100.0 # Susceptible

# ╔═╡ 522e1306-ecb4-4a13-b215-b6057c25ce63
E_begin = 10.0 # Exposed

# ╔═╡ 6987ba33-5a19-4cc6-9b9e-5ff230941eed
I_begin = 1.0 # Infected-Infectious

# ╔═╡ d4fa6213-f0e1-44f9-b12e-17c5a3fd6aab
R_begin = 0.0 # Recovered

# ╔═╡ 5091a2aa-8b4f-4c08-913b-b4244f92009b
u_begin = [S_begin, E_begin, I_begin, R_begin]

# ╔═╡ f2db2479-6524-4970-9493-9e9f806dc0c6
md"### p: Parameters"

# ╔═╡ c378aee2-ccb8-468c-b47c-ee57e627a51f
md"Note: these initial values are abstract placeholders to be used for model-building purposes only."

# ╔═╡ 96cff75f-3809-48a7-a5c1-1b26ea963e60
Lambda = 17 / 1000 # per capita birth rate

# ╔═╡ c0e50ec3-a0b6-44a5-a5ae-e339d301161a
mu = 8 / 1000 # per capita natural death rate

# ╔═╡ 187b676e-8e74-49e8-87e4-8ae1e8d90e2c
@bind beta Slider(0.0:0.01:1.0, 90 / 100, true) # probability of disease transmission per contact

# ╔═╡ 19798c3b-c191-4f01-b82b-6b28fa9a5501
@bind epsilon Slider(0.0:0.001:0.5, 25 / 100, true) # rate of progression from exposed to infectious

# ╔═╡ d92047a4-4a0e-4fb6-8d20-c68511fcc68a
@bind gamma Slider(0.0:0.001:0.5, 10 / 100, true) # recovery rate of infectious individuals

# ╔═╡ 9acf7050-ad71-4504-89f0-51b4cfd3bc6a
@bind alpha Slider(0.0:0.001:0.05, 1 / 1000, true) # virus-induced average fatality rate

# ╔═╡ 26cca7d9-0c9e-495b-86ba-8eee7820704e
p = [Lambda, mu, beta, epsilon, gamma, alpha]

# ╔═╡ 6f9c72ad-bd0a-4bee-bafb-b92680236629
md"### t: Time"

# ╔═╡ 1142df77-e8da-4551-afa5-8b29ed0d43be
md"Note: these initial values are abstract for demonstration purposes only."

# ╔═╡ 241dde8d-6bf2-48f8-8c46-6d8951a8ce7e
t_begin = 0.0

# ╔═╡ 8b4d1b78-74f3-4955-9fea-e7793afc2639
t_end = 40.0

# ╔═╡ a2807fdf-1853-45a1-8b98-e6af77d4c488
tspan = (t_begin, t_end)

# ╔═╡ 0986b87c-8d70-4d36-9cd1-e7e09e8230b9
md"## Define System of First Order ODEs Problem"

# ╔═╡ ae261c8a-c304-4409-ac23-b1fa7ccfe348
prob = ODEProblem(SEIR, u_begin, tspan, p)

# ╔═╡ c6fc4885-9272-4532-a672-abd347f673f4
md"## Solve Problem"

# ╔═╡ fe6e31fe-3f4f-47db-9149-dc3149dc64d8
sol = solve(prob)

# ╔═╡ f1358e0c-2552-418d-a41a-9dcbf6429bf8
md"## Plot Solution"

# ╔═╡ 9d504fd2-ae05-4212-86f5-2c6cbdd8f647
plot(sol,
	legend = (0.72, 0.58),
	linewidth = 2,
	title = "SEIR Model",
	xaxis = "Time",
	yaxis = "Population",
	labels = ["Susceptible" "Exposed" "Infected-Infectious" "Recovered"],
	formatter = :plain,
	widen = true,
	xlims = (0.0, 40.0),
	ylims = (0.0, 100.0)
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "60419609424d02ce5c3e0229634bdbdee01fc728"
