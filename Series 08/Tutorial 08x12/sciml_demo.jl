...
# ╔═╡ 29123816-500d-48fc-99b2-995f84535022
using DifferentialEquations, DiffEqFlux, Flux

# ╔═╡ 9a8e20c3-a325-4692-af98-5c54938ba4d4
using AdvancedHMC, MCMCChains, Plots

# ╔═╡ 964d4b73-92f3-41c2-b6e3-698a2f90cd8c
using ComponentArrays, Random

# ╔═╡ 1e3ebf20-cf2d-11ed-163c-fd04d0cc9c35
md"# SciML Demo"

# ╔═╡ 6d1f456e-a987-4945-915d-80219ca492fb
md"Source: [SciML Documentation](https://docs.sciml.ai/Overview/stable/showcase/bayesian_neural_ode/#bnode)"

# ╔═╡ 34770e85-e070-4d3d-8b61-8f23d7d6f2dd
md"## Load Packages"

# ╔═╡ 153f33fb-bd07-4f3a-b120-be13965f394a
md"Note: loading every package may take a few minutes."

# ╔═╡ 590b8d55-d7fe-4961-a32f-34f1bce2e9fb
md"## Step 1: Set Up"

# ╔═╡ edadf027-2f55-4e1a-8827-a8ecc4f6f776
u0 = [2.0; 0.0]

# ╔═╡ d0a06642-a42e-404b-955c-19457b75f02d
datasize = 40

# ╔═╡ 2c3becdf-63c3-4fbc-914e-ccd81d2cc5ae
t_begin = 0.0

# ╔═╡ b44be53d-ae5b-4099-ab15-5644fb461159
t_end = 1.0

# ╔═╡ 06e8e8ca-59a9-4c9c-ac70-06e44bae5e38
tspan = (t_begin, t_end)

# ╔═╡ bc8c3003-7641-4c7a-ba5c-e4ec36d8f8a9
tsteps = range(t_begin, t_end, length = datasize)

# ╔═╡ 4ac70d0b-9712-4763-8939-5a299ff91c1b
function trueODEfunc(du, u, p, t)
	true_A = [-0.1 2.0; -2.0 -0.1]
	du .= ((u .^ 3)' * true_A)'
end

# ╔═╡ 83ee293f-285d-42e3-935a-e3d47cd4dde9
prob_trueode = ODEProblem(trueODEfunc, u0, tspan)

# ╔═╡ 49393cc9-92de-42a3-bbc6-234d3cab7167
ode_data = Array(
	solve(prob_trueode, Tsit5(), saveat = tsteps)
)

# ╔═╡ da200bff-4eb8-453b-8c19-7b9d66bfc757
md"### Plot Data"

# ╔═╡ eba15498-67b8-436c-b4ff-8bb0a3f8185c
...
# ╔═╡ 1c5a2ce1-6f5c-47f6-8cc9-4800a41ffccc
scatter!(plot1, tsteps, ode_data[2, :],
	color = :blue,
	label = "Data: Var2"
)

# ╔═╡ 70a04f67-c1de-40f2-bfb1-7fcc3ac49c04
...
# ╔═╡ 4982e286-acc7-4a99-bb9c-de8e229dcea5
md"## Step 2: Define the Neural ODE Architecture"

# ╔═╡ 39999b60-a5d6-4be2-941f-7e763c10b8ae
dudt2 = Flux.Chain(x -> x .^ 3,
	Flux.Dense(2, 50, tanh),
	Flux.Dense(50, 2)
) |> f64

# ╔═╡ 5d32bf0e-4141-4a04-87a1-dae62133b2cf
prob_neuralode = NeuralODE(
	dudt2, tspan, Tsit5(), saveat = tsteps
)

# ╔═╡ b44745d2-4fb5-4197-9bb5-e69d894dcd9c
rng = Random.default_rng()

# ╔═╡ 240a2c7b-9535-4e90-85ce-6e16c2ab7202
p = Float64.(prob_neuralode.p)

# ╔═╡ 936eb0ad-2448-424e-91cf-f2c08156ae8c
md"## Step 3: Define Loss Function for Neural ODE"

# ╔═╡ 094652bf-ddeb-48dc-9886-a5eaa1f12f91
function predict_neuralode(p)
	Array(prob_neuralode(u0, p))
end

# ╔═╡ 50dd477c-6268-4cb7-9200-a4ec57ec4729
...
# ╔═╡ 81b39f99-49e5-46f1-8fdf-c3f9effec514
md"## Step 4: Integrate Bayesian Estimation Workflow (as prescibed by the AdvancedHMC interface) with the NeuralODE defined above"

# ╔═╡ f6b3be9c-2ef5-4d70-a25b-ab447de03b09
loss(theta) = -sum(abs2, ode_data .- 
	predict_neuralode(theta)
) - sum(theta .* theta)

# ╔═╡ ce9dbdc7-8d99-4c8d-bbfb-2224e362932d
...
# ╔═╡ 065907aa-5575-47d1-9ecc-69e2624de470
metric = DiagEuclideanMetric(length(p))

# ╔═╡ 293015b7-d1b3-426c-898f-3ca226c6a65e
hamiltonian = Hamiltonian(metric, loss, dloss_dtheta)

# ╔═╡ d1c5ed4a-0997-45d7-aa91-4337c0813356
md"Note: finding the integrator may take a few minutes."

# ╔═╡ cd4b6894-02bb-46c7-98cf-e1d92a03a798
integrator = Leapfrog(
	find_good_stepsize(hamiltonian, p)
)

# ╔═╡ f702733d-79f4-4952-9513-6b3f91078d73
proposal = AdvancedHMC.NUTS{
	MultinomialTS, GeneralisedNoUTurn
}(integrator)

# ╔═╡ b350e121-27f6-4b68-bbe2-919cd23a0f05
adaptor = StanHMCAdaptor(
	MassMatrixAdaptor(metric),
	StepSizeAdaptor(0.45, integrator)
)

# ╔═╡ 85f9d452-d54d-4724-bb31-c04de0153b31
md"Warning: drawing 500 samples may take over 30 minutes to run."

# ╔═╡ 2525fbbd-1fd2-49e6-8bf9-443d4ef9a112
n_samples = 50

# ╔═╡ dbffd6a3-aaad-4390-9186-3b966c667b45
samples, stats = sample(hamiltonian, proposal, p,
	n_samples, adaptor, n_samples;
	progress = true
)

# ╔═╡ 50d6611a-7165-4f15-93dd-4558fd1c8da4
md"## Step 5: Analysis"

# ╔═╡ e11cd7f8-1e12-498c-9554-47fed1076e0d
samples_hcat = hcat(samples...)

# ╔═╡ e3f2406e-4ee8-4fbf-bc16-f4d9373a5e3d
md"### Plot 1: Time Series"

# ╔═╡ 9c08f7eb-9b1f-41b1-a6e7-81fe6db40ce9
for k in 1:300
	resol = predict_neuralode(
		samples_hcat[:, 10:end][:, rand(1:40)]
	)
...
# ╔═╡ f47c6f13-acdc-4a8f-ba67-ae5af27a927a
plot1

# ╔═╡ 04c99e59-0b8a-4284-9e80-84a6800769ad
losses = map(x -> loss_neuralode(x)[1],
	eachcol(samples_hcat)
)

# ╔═╡ fc6cf020-53c3-4433-9c7b-69605f0e5a4b
idx = findmin(losses)[2]

# ╔═╡ 59b3ee2b-2671-4d19-adbc-de38af2c525e
prediction = predict_neuralode(
	samples_hcat[:, idx]
)

# ╔═╡ a0b6a263-b114-49f2-a1bf-2bffc4cd192e
...
# ╔═╡ f8daebb0-c956-4efa-9b73-1123cb592c46
...
# ╔═╡ e90da5b7-a9ff-46ca-a5e0-a25a3c1cef28
savefig(plot1, "plot1.svg")

# ╔═╡ 4a508978-e786-4258-8a0b-be41f9efaf15
md"### Plot 2: Phase Space"

# ╔═╡ 0151fda7-13da-4b7c-a78a-ab3f681edaa8
for k in 1:300
	resol = predict_neuralode(
		samples_hcat[:, 10:end][:, rand(1:40)]
	)
	plot!(plot2, resol[1, :], resol[2, :],
		alpha = 0.04,
		color = :red,
		label = ""
	)
...
# ╔═╡ 4dbc98c8-367b-4f86-8b6c-e86edef1d135
plot2

# ╔═╡ ec4e9847-4bea-4b95-906b-bc9cc72d001f
plot!(plot2, prediction[1, :], prediction[2, :],
	color = :black,
	linewidth = 2,
	label = "Best fit prediction"
)

# ╔═╡ 990bbad2-abe9-440d-9d14-b7fdeb18fb90
savefig(plot2, "plot2.svg")

# ╔═╡ 1fe39856-fa17-4258-aac0-6f65171e211e
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "aa8a548a7b6be4b8c6aa07c7953eaca0063eb839"
...
[[deps.DiffEqFlux]]
deps = ["Adapt", "Cassette", "ChainRulesCore", "ConsoleProgressMonitor", "DataInterpolations", "DiffEqBase", "DiffResults", "Distributions", "DistributionsAD", "Flux", "ForwardDiff", "Functors", "LinearAlgebra", "Logging", "LoggingExtras", "Lux", "NNlib", "Optim", "Optimization", "OptimizationFlux", "OptimizationOptimJL", "OptimizationPolyalgorithms", "Printf", "ProgressLogging", "Random", "RecursiveArrayTools", "Reexport", "Requires", "SciMLBase", "SciMLSensitivity", "StaticArrays", "TerminalLoggers", "Zygote", "ZygoteRules"]
git-tree-sha1 = "a4af1ccf4f94ea0ed20423cd7baa90a9100daf47"
