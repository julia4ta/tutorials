...
# ╔═╡ 30cd5e81-3108-4609-9042-8838f0ccbadb
using Turing, MCMCChains, StatsPlots, PlutoUI

# ╔═╡ 6b83d6b0-c430-11ed-262d-3f9570f0c9f4
md"# Coin Toss Experiment"

# ╔═╡ a6dc7dbe-099b-47e5-af2b-d46c18a6601c
...
# ╔═╡ 0bc48f85-e70b-4a4d-a460-a46296e01cd4
md"Tutorial Code Source (with some modifications): [Turing.jl Tutorials](https://turinglang.org/v0.24/tutorials/00-introduction/)"

# ╔═╡ 15615dba-6c42-4be9-8c97-b1e09d85b8ba
md"## Load Packages"

# ╔═╡ 5fd45e51-97b5-4fba-bf0f-36a9305c2e6c
md"## Part 1: Without Turing.jl"

# ╔═╡ 0d454f61-70a4-49f1-9252-0ff2dea5a9bb
md"### Make Observations"

# ╔═╡ 9b1e5e9b-7344-4c77-b887-0712dc5460e2
p_true = 0.5

# ╔═╡ 3e93b70c-abaa-4658-904a-5856fb1f6d6d
tosses = 100

# ╔═╡ 20ad32cc-cd45-4f12-abbc-a300df8f0382
data = rand(Bernoulli(p_true), tosses)

# ╔═╡ 5765989f-8c58-48a5-9769-4e7b2009577b
p_data = sum(data) / tosses

# ╔═╡ 0c80627d-ef9b-4d9e-af93-d4a9b3b6db53
prior_belief = Beta(1, 1)

# ╔═╡ 4f44e353-079a-40f4-98ec-3ad593a95bc6
md"### Update Belief"

# ╔═╡ 749a2e9b-6f46-4658-9825-433657c03c72
function updated_belief(prior_belief, data)
	heads = sum(data)
	tails = length(data) - heads
	return Beta(prior_belief.α + heads, prior_belief.β + tails)
end

# ╔═╡ 42412f26-6175-48e8-91cf-3d88cec2b61e
updated_belief(prior_belief, data)

# ╔═╡ 341b5053-5172-4ac2-b231-5f6610b8092d
md"### Visualize Probability Distribution"

# ╔═╡ 0e460205-55e1-4bf0-b2fd-746b96ab0aaf
md"Observation(s): $(@bind observation Slider(1:tosses, 1, true))"

# ╔═╡ aa60b5e2-1215-4740-a4d9-f9d86983c41a
updated_belief(prior_belief, data[1:observation])

# ╔═╡ f6274237-3fc7-47be-be35-4e1f170999bf
begin
	plot(updated_belief(prior_belief, data[1:observation]),
		legend = false,
		title = "Updated Belief After $observation Observation(s)",
...
# ╔═╡ 346f54af-b90f-44be-875d-21fb909b80d9
md"## Part 2: With Turing.jl"

# ╔═╡ 570e0297-3ec2-4d45-b2cb-477132656dc5
md"### Define Model Based on Data"

# ╔═╡ e5f80779-0897-4646-bb0e-4274377553e2
@model function cointoss(; tosses)
	p ~ Beta(1, 1)
	y ~ filldist(Bernoulli(p), tosses)
	return y
end

# ╔═╡ 7e467601-bdab-478e-bef1-b36f52edd169
cointoss(y) = cointoss(; tosses = length(y)) | (; y)

# ╔═╡ e97e81d2-4e7a-4bef-aecd-286dbc345a55
rand(cointoss(; tosses))

# ╔═╡ 083454c9-02e3-428f-8596-79192304abab
model = cointoss(data)

# ╔═╡ b1bd45eb-639e-4bf3-a511-fac1dee54638
md"### Approximate Posterior Distribution"

# ╔═╡ 33aec872-54c3-42d7-9cd0-c6d18c7cf000
sampler = NUTS()

# ╔═╡ a65bef45-e18a-4cdb-8586-46e366af55d3
md"Samples: $(@bind samples Slider(50:10:2_000, 50, true))"

# ╔═╡ aa8ab117-ee34-4eb9-ade8-aae092104559
chain = sample(model, sampler, samples; progress = false)

# ╔═╡ 8f111902-c43b-46a6-8534-6580e0083370
sum_stats = summarystats(chain)

# ╔═╡ 705faa93-cbbb-4ba5-9986-c46b9e0f4d70
mu = sum_stats[1, :mean]

# ╔═╡ ebf76b40-93b1-4c20-a2dd-b5bb745f44d4
md"### Visualize Probability Distribution"

# ╔═╡ e31c2104-1d48-41ab-8b9d-b63b3d72c34b
plot(chain)

# ╔═╡ 6ea85dfe-4fff-46ed-90d3-77e992c298c0
begin
	histogram(chain, xlims = (0.2, 0.8))
	vline!([mu], linewidth = 2)
end

# ╔═╡ d5df7bc7-6063-413c-9c4b-668b28ef7f1e
begin
	# approximate posterior distribution using Turing
	density(chain,
		legend = false,
		linewidth = 2,
		color = :blue
	...
	# mean of samples
	vline!([mu], linewidth = 2, color = :blue)
	# posterior distribution without using Turing
	plot!(0:0.01:1,
	...
	vline!([p_true], color = :red)
end

# ╔═╡ ef2c1a29-6fa7-49c4-ad77-2ebd6da38623
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "3364a101214856ba27b4ac8dee6940db0127b5c2"
